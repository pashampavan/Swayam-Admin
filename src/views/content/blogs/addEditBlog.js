...
const AddEditBlog = () => {
    ...
    if (id !== 'b1') {
      const fetchBlog = async () => {
        try {
          ...
          setBlogThumbnail(blogData.blogthumbnail);
  ...
  return (
    ...
    <Paper elevation={3} sx={{ p: 2, maxWidth: '80%', margin: '150px auto' }}>
      <Typography variant="h4">{id === 'b1' ? 'Add' : 'Edit'} Blog</Typography>
      <Box my={1}>
        <TextField
          fullWidth
          label="Blog Title"
      ...
      <Box my={2}>
        <Button variant="contained" onClick={handleSaveBlog} disabled={blogContent.length === 0}>
